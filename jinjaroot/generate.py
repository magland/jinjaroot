...
def process_jinjaroot_dir(*, src_dirpath: str, dst_dirpath: str, template_data: dict, verify_only: bool, force: bool):
    ...
    for fname in template_fnames:
        ...
        if os.path.isdir(template_path):
            if not os.path.exists(dest_path):
                os.mkdir(dest_path)
            process_jinjaroot_dir(src_dirpath=template_path, dst_dirpath=dest_path, template_data=template_data, verify_only=verify_only, force=force)
            ...
            _check_and_write_file(dst_path=dest_path, gen_code=gen_code, verify_only=verify_only, force=force, template_path=template_path)

def _check_and_write_file(*, dst_path: str, gen_code: str, verify_only: bool, force: bool, template_path: Union[str, None]):
    ...
    if gen_code != dst_code:
        ...
        if (template_stat is not None) and (dst_stat is not None) and not force:
            if dst_stat.st_mtime - template_stat.st_mtime > 1:
                raise Exception(f'File is newer than template (you can use the --force option if you are sure you want to overwrite): {dst_path}')
...
def process_dir(*, dirpath: str, template_data: dict, verify_only: bool, force: bool):
    ...
    if os.path.isdir(jinjaroot_dirpath):
        process_jinjaroot_dir(src_dirpath=jinjaroot_dirpath, dst_dirpath=dirpath, template_data=template_data2, verify_only=verify_only, force=force)
        ...
        if os.path.isfile(file_path):
            if ends_with(file_path, ['.ts', '.tsx', '.js', '.jsx', '.py', '.cfg', '.md', '.sh', '.yml', '.yaml']):
                ...
                if os.path.exists(template_path):
                    ...
                    _check_and_write_file(dst_path=file_path, gen_code=gen_code, verify_only=verify_only, force=force, template_path=template_path)
            if ends_with(file_path, ['.json']):
                ...
                if os.path.exists(substitute_template_path):
                    ...
                    _check_and_write_file(dst_path=file_path, gen_code=gen_code, verify_only=verify_only, force=force, template_path=None)
        elif os.path.isdir(file_path):
            process_dir(dirpath=file_path, template_data=template_data2, verify_only=verify_only, force=force)
...
def generate(*, verify_only: bool=False, force: bool=False):
    template_data = {}
    process_dir(dirpath='.', template_data=template_data, verify_only=verify_only, force=force)

def verify():
    generate(verify_only=True, force=False)
